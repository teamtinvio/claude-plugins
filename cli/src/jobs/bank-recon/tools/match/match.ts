...
export function matchBankRecords(input: BankMatchInput): BankMatchResult {
  ...
  for (const ri of [...recordPool]) {
    ...
    const subsets = subsetSumDFS(subCandidates, Math.abs(r.cents), effectiveTolCents, maxK, findAll);
  ...
  for (const ti of [...txnPool]) {
    ...
    const subsets = subsetSumDFS(subCandidates, Math.abs(t.cents), effectiveTolCents, maxK, findAll);
    if (subsets.length === 0) continue;
